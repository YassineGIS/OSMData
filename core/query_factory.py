    ...
    def check_parameters(self):
        if self.__nominatim and self.__bbox:
            raise QueryFactoryException(
                suffix=tr('OSMData', 'nominatim OR bbox, not both'))

        if not self.__key:
            raise QueryFactoryException(suffix=tr('OSMData', 'key required'))

        if len(self.__osm_objects) < 1:
            raise QueryFactoryException(
                suffix=tr('OSMData', 'osm object required'))

        for osmObject in self.__osm_objects:
            if osmObject not in QueryFactory.OSM_TYPES:
                raise QueryFactoryException(
                    suffix=tr('OSMData', 'wrong OSM object'))

        if self.__is_around and not self.__distance:
            raise QueryFactoryException(
                suffix=tr('OSMData', 'No distance provided with "around".'))

        if self.__is_around and not self.__nominatim:
            raise QueryFactoryException(
                suffix=tr('OSMData', 'No nominatim provided with "around".'))
