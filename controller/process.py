...
from OSMData.core.query_factory import QueryFactory
from OSMData.core.utilities.tools import tr
from OSMData.core.exceptions import FileOutPutException, OsmDriver
from OSMData.core.api.connexion_oapi import ConnexionOAPI
from OSMData.core.parser.osm_parser import OsmParser
from OSMData.core.utilities.operating_system import get_default_encoding
from OSMData.core.utilities.utilities_qgis import is_osm_driver_enabled
from OSMData.core.utilities.tools import get_setting
from OSMData.core.query_parser import prepare_query
        ...
        if not output_dir:
            ...
            if output_format == "shape":
                temp_file = tempfile.NamedTemporaryFile(
                    delete=False, suffix="_" + layer + "_osmdata.shp")
            else:
                # We should avoid this copy of geojson in the temp folder
                temp_file = tempfile.NamedTemporaryFile(
                    delete=False, suffix="_" + layer + "_osmdata.geojson")
...
def open_file(
    ...
    num_layers = 0
    if output_format == "shape":
        dialog.set_progress_text(tr("OSMData", u"From GeoJSON to Shapefile"))
    ...
    for i, (layer, item) in enumerate(layers.iteritems()):
        ...
        if item['featureCount'] and layer in output_geom_types:
            ...
            actions.addAction(
                ...
                'JOSM',
                'from OSMData.CoreOSMData.Actions import Actions;'
            ...
            for link in ['url', 'website', 'wikipedia', 'ref:UAI']:
                if link in item['tags']:
                    ...
                    actions.addAction(
                        QgsAction.GenericPython,
                        link,
                        'from OSMData.core.actions import Actions;'
...
def process_query(
    ...
    # Prepare outputs
    dialog.set_progress_text(tr("OSMData", u"Prepare outputs"))

    # Replace Nominatim or BBOX
    query = prepare_query(query=query, extent=bbox)

    # Getting the default overpass api and running the query
    server = get_setting('defaultOAPI')
    dialog.set_progress_text(tr("OSMData", u"Downloading data from Overpass"))
    QApplication.processEvents()
